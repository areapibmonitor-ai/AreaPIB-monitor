...
- On removals, classifies as EXPIRED / REPLACED / OTHER REMOVAL.
...
def http_get_json(url: str, timeout: int = TIMEOUT_SECS) -> Tuple[dict, str]:
    req = urllib.request.Request(
        url,
        headers={
            "User-Agent": "area24-monitor/1.4 (+local)",
...
def build_q_line(n: dict) -> Optional[str]:
    ...
    if not (fir and subject and condition and traffic and purpose and scope and
        ...
        return None
    return f"{fir}/{subject}{condition}/{traffic}/{purpose}/{scope}/{int(lower):03d}/{int(upper):03d}/{coord}{int(radius):03d}"
...
def build_aftn(n: dict, replaced_ref: Optional[str] = None) -> str:
    ...
    # If NOTAMR and replaced_ref not provided, derive from JSON
...
def detect_replaced_target(n: dict) -> Optional[str]:
    """Return 'A####/YY' for the NOTAM being replaced (for NOTAMR)."""
    # Preferred: referenceId {series, number, year}
    ref = n.get("referenceId")
    if isinstance(ref, dict):
        s = ref.get("series"); num = ref.get("number"); yr = ref.get("year")
        if isinstance(s, str) and isinstance(num, int) and isinstance(yr, int):
            return f"{s.upper()}{num:04d}/{yr % 100:02d}"
    # Other fields sometimes used by sources
...
def main():
    parser = argparse.ArgumentParser(description="Monitor area24 NOTAMs")
    ...
    # Build helper: index of replacements among added NOTAMs of type R
    ...
    # Additions (skip replacers already shown)
    for k in sorted(added_keys):
